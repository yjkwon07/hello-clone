...
import { requestAddChannel, useListWorkspaceChannel } from '@API/workspaceChannel';
...
const AddChannelModal: VFC<Props> = ({ show, onCloseModal }) => {
  ...
  const onCreateChannel = useMemo(() => {
    return checkSubmit(async (formData) => {
      ...
      setAddSuccess(false);
      try {
        const { data } = await requestAddChannel({ name: formData.channel }, { workspace });
        const listData = channelData?.length ? channelData.concat(data) : [data];
        await mutate(listData, false);
        setAddSuccess(true);
        onCloseModal();
      } catch (err) {
